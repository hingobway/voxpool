...
public:
                        ...
                        const juce::Slider::SliderStyle style, juce::Slider &slider) override
  {
    ...
    // find dimensions
    auto ix = int(x + ((float)width / 2) - (sliderWidth / 2));
    ...
    if (!(val <= 1.0 && val >= 0.0))
      val = 0.0;
    auto fill = (int)roundf((1.0f - val) * height);
    ...
    thumb.addRoundedRectangle(
        (float)x + ((float)width - thumbWidth) / 2.0f,
...
public:
                          ...
                          bool /*shouldDrawButtonAsHighlighted*/, bool /*shouldDrawButtonAsDown*/) override
  {
    ...
    if (toggleState)
    {
      ...
      svgOn->drawAt(g, float(bcx - cx), float(bcy - cy), 1.0);
    }
    else
    {
      ...
      svgOff->drawAt(g, float(bcx - cx), float(bcy - cy), 1.0);
    }
  }
